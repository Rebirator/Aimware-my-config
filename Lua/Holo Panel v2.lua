...
local function Drawholopanel( x1, y1, x2, y2, clr, sclr, tclr, frclr, fclr, sxclr, svclr, eclr )
    ...
    ----------------------------------------------------------------
    local hitchance   = gui_GetValue( "rbot.accuracy.weapon." .. WEAPON_CURRENT_GROUP .. ".hitchance" );
    draw_ShadowText( left_x, left_y, "HIT CHANCE: " .. hitchance, sclr, tclr );
    left_y = left_y + 10;
    ----------------------------------------------------------------
    local mindamage   = gui_GetValue( "rbot.accuracy.weapon." .. WEAPON_CURRENT_GROUP .. ".mindmg" );
    ...
    local dt_mode        = gui_GetValue( "rbot.accuracy.weapon." .. WEAPON_CURRENT_GROUP .. ".doublefire" )
    if dt_mode ~= 0 then
        if dt_mode == 1 then
...
callbacks.Register( "Draw", "HoloPanelDraw", function( )
    ...
    if WEAPON_CURRENT_GROUP == "global" or WEAPON_CURRENT_GROUP == "knife" then return end;
    if not entities_GetLocalPlayer( ) or entities_GetLocalPlayer( ):IsAlive( ) then return end;
    if muzzle_pos_x == nil or muzzle_pos_y == nil then return end;
